...
/*
  ...
  uint8_t cur_addr_reads = 0;
  uint8_t cur_addr_recv = 0;
  ...
  while( writes < l_size ){
    for(cur_core = 0; cur_core < num_cores; cur_core++){
      if( q_start - q_end != 0){
        ...
        // send a read request as long as we havent't sent one to
	  ...
	  tag++;
	  if( tag > 1023 ) tag=0;
          if( cur_addr_reads >= stencil_point-1){
	    sreads_sent++;
	  }
          break;
        case HMC_STALL:
      ...
      for( i=0; i<hmc->num_links; i++ ){
        ...
        while( ret != HMC_STALL ){
          ...
          if( ret == 0 ){
            ...
            if( d_type == RD_RS){
              if( ++cur_addr_recv >= stencil_point-1){
                uint32_t i;
