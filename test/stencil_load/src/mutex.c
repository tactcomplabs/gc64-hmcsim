 ...
 * _MUTEX_C__
 *
 * HMCSIM MUTEX TEST DRIVER
 *
 * Attempts to initialize a HMC instantiation
 * and execute a mutex test
 *
 */
...
static int load_cmc_libs( struct hmcsim_t *hmc, char *libsrc ){
  ...
  int i       = 0;
  char *lib   = NULL;
  char *mlibs[] = { "/stencil_load/stencil_load_1d_1.so",
                    "/stencil_load/stencil_load_2d_1.so"};
  /* ---- */
...
extern int main( int argc, char **argv ){
  ...
  while(( ret = getopt( argc, argv, "b:c:d:hl:m:n:q:v:x:L:" )) != -1 )
  {
    switch( ret )
    {
      case 'b':
