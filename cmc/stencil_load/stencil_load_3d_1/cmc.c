...
static char *__op_name      = "STENCIL_LOAD_3D_1";
...
*/
static hmc_rqst_t __rqst    = CMC22;
...
extern int hmcsim_execute_cmc(  void *hmc,
  ...
  switch (neighborhood_type){
    ...
    case NarrowVonNeumann:
	  ...
	  fetch_addr = addr + (i*y_width*x_width*data_size);
          if( (*readmem)(l_hmc, fetch_addr, &data, 1) != 0 ){
            /* error in data read */
            return -1;
          }
