...
static char *__op_name      = "STENCIL_READ_1D_1";
...
*/
static hmc_rqst_t __rqst    = CMC20;
...
void pack_data(uint64_t data, uint64_t *payload, uint8_t count, uint8_t data_width){
  ...
  payload[payload_index] += (data & ((int) pow(2,data_width)-1)) << (payload_count * data_width);
}
...
uint8_t stencil_point(uint8_t center_incl, uint8_t neighborhood_type, uint8_t radius, uint8_t dim){
  /* All neighborhood types are isomorphic in 1D, maybe look into alternative
   * neighborhoods for 1D? */
...
extern int hmcsim_execute_cmc(  void *hmc,
  ...
  __dynamic_rsp_len = point * data_width;
